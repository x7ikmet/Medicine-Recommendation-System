...
public class XGBoostHelper
{
    
    private XGBClassifier _model;
    private Dictionary<string,int> _symptomsDict;
    private Dictionary<int, string> _diseasesList;

    public XGBoostHelper(string modelPath)
    {
        _model = XGBClassifier.LoadFromFile(modelPath);
        //MessageBox.Show("Model loaded successfully.");

        _symptomsDict = new Dictionary<string, int>
        {
        ...
        _diseasesList = new Dictionary<int, string>
        {
    ...
    public List<(string Disease, float Probability)> Predict(string[] symptoms)
    {
        // Initialize input vector with zeros
        float[] inputVector = new float[_symptomsDict.Count];

        // Set 1 for symptoms present
        foreach (var symptom in symptoms)
        {
            if (_symptomsDict.TryGetValue(symptom, out int index))
            {
                inputVector[index] = 1f;
            }
        }

        // Wrap inputVector in a jagged array for DMatrix (one sample)
        float[][] inputJagged = new float[1][] { inputVector };

        using var dmatrix = new DMatrix(inputJagged);

        // Predict probabilities (returns float[][], one row per sample)
        var probabilities = _model.PredictProbability(dmatrix);

        // Map probabilities to disease names and round to 2 decimals percentage
        var results = new List<(string Disease, float Probability)>();
        
        for (int i = 0; i < probabilities.Length; i++)
        {
            if (_diseasesList.TryGetValue(i, out string diseaseName))
            {
                float probPercent = (float)Math.Round(probabilities[i][1] * 100, 2);
                results.Add((diseaseName, probPercent));
            }
        }
